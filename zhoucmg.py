...
class ExecutionBot(Management):
    ...
    defffff start_task(self, sym, action, size, max_t, regime, start_t):
        self.start()
    ...
    def task_complete(self, pv, qty, end_t, slices):
        ...
        self.stop(self.stat, cfg_mode, log=True)
...
if __name__ == "__main__":
    ...
    myargparser.add_argument('--strategy', type=str, const="PoV", nargs='?', default="AggressiveWithSlice")
    ...
    #user_bot = ExecutionBot_user(strategy, starting_money, market_event_securities, market_event_queue, securities,
    #                        cfg_mode, cfg_rbmq, cfg_kdb,
    #                        host, bot_id)

    #user_bot.start_task(args.symbol, args.action, args.size, args.maxtime, args.regime, start_t)
    ...
    #elif strategy == 'User_bot':
    #    num_slices, pv, qty = user_bot.ptb_orders(args.size, args.action, args.maxtime, num_slices, beta)
